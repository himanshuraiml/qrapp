...
BEGIN
  ...
  SELECT
    ...
    a.session, a.date, a.marked_by_name, a."timestamp", a.batch
  FROM attendance a
  WHERE
...
BEGIN
  ...
  -- 1. Count of present sessions (distinct to guard against duplicates)
...
-- ─────────────────────────────────────────
-- get_unified_student_roster
-- Per-student attendance roster with overall and date-range stats.
-- Conducted sessions are computed from profiles.batch/section (not
-- attendance.batch) so that old records without batch set are counted
-- correctly instead of inflating the percentage above 100%.
-- ─────────────────────────────────────────
...
BEGIN
  RETURN QUERY
  WITH
  -- Conducted sessions derived from profiles.batch/section so old attendance
  -- rows that lack batch='...' are still counted correctly.
