...
import { exportBatchRosterToExcel, exportBatchRosterToPDF } from '@/lib/export'
import type { BatchRosterRecord } from '@/types'
...
export default function BatchRosterTable({ rows, loading, date, session }: Props) {
  ...
  const groups = useMemo(() => {
    ...
    rows.forEach((r) => {
      const key = r.batch
      if (!map.has(key)) map.set(key, { batch: r.batch, rows: [] })
  ...
  async function handleExcel() {
    setExporting(true)
    await exportBatchRosterToExcel(rows, date, session)
    setExporting(false)
  }

  async function handlePDF() {
    setExporting(true)
    await exportBatchRosterToPDF(rows, date, session)
    setExporting(false)
  }
