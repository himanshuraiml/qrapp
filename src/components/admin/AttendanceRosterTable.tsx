...
import { exportRosterToExcel, exportRosterToPDF, exportRosterMultiToExcel, exportRosterMultiToPDF } from '@/lib/export'
import type { RosterRecord, RosterMultiRecord } from '@/types'

const SESSION_COLS: Array<{ key: keyof RosterMultiRecord; label: string; isFN: boolean }> = [
...
export default function AttendanceRosterTable({ rows, multiRows, loading, date, session, showExport = true }: Props) {
  ...
  // Group multi-session rows by dept+section
  const multiGroups = useMemo(() => {
    const map = new Map<string, { dept: string; section: string; rows: RosterMultiRecord[] }>()
    multiRows.forEach((r) => {
      const key = `${r.department}__${r.section}`
      if (!map.has(key)) map.set(key, { dept: r.department, section: r.section, rows: [] })
      map.get(key)!.rows.push(r)
  ...
  async function handleExcel() {
    setExporting(true)
    if (isMulti) await exportRosterMultiToExcel(multiRows, date)
    else await exportRosterToExcel(rows, date, session)
    setExporting(false)
  }

  async function handlePDF() {
    setExporting(true)
    if (isMulti) await exportRosterMultiToPDF(multiRows, date)
    else await exportRosterToPDF(rows, date, session)
    setExporting(false)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Section groups accordions */}
      <div className="space-y-4">
        {isMulti
          ? multiGroups.map((g) => (
              <MultiGroup
                key={`${g.dept}__${g.section}`}
                g={g}
...
function SingleGroup({
  ...
  g: { dept: string; section: string; rows: RosterRecord[] }
...
function MultiGroup({
  ...
  g: { dept: string; section: string; rows: RosterMultiRecord[] }
  ...
  const sessionStats = activeCols.map(({ key: colKey, label, isFN }) => ({
    label,
    isFN,
    present: g.rows.filter((r) => r[colKey]).length,
    total: g.rows.length,
  }))
            ...
            <tbody className="divide-y divide-slate-100 bg-white/20">
              {g.rows.map((r) => (
                <tr key={r.student_id} className="hover:bg-slate-50/30 transition-colors">
                  ...
                  <td className="px-4 py-2.5 font-bold text-slate-800 text-sm">{r.name}</td>
                  <td className="px-4 py-2.5 text-center font-bold text-slate-600">{r.year}</td>
                  {activeCols.map(({ key: colKey, label }) => (
